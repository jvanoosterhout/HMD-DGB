...
class PinKeeper(object):
    ...
    def GetPin(self, config:PinModel):
        ...
        if type(pin_id) == bool: # is pin config, but does not exist jet
            self.logger.info('Is config for pin{}, but does not exist jet.'.format(config.pin))
            if self.SetPin(config):
                pin_id = self.DoIExist(config)
                return self.PinList[pin_id].GetPinValue()
        else: # is pin config, and it exists --> ask its value
            self.logger.info('Is config for pin {}, and it exists, therefore requesting its value.'.format(config.pin))
            return self.PinList[pin_id].GetPinValue()
    ...
    def SetPin(self, config:PinModel) -> bool:
        ...
        """
        pin_id = self.DoIExist(config)
        if type(pin_id) == bool: # pin does not exist jet
            if config.ptype == PinType.pinnwayout.value:
                ...
                for p in config.pin_list:
                    if p >= 0 and not p == config.pin:
                        if type(self.DoIExist(PinModel({ "pin": p, "ptype": PinType.pinout.value}))) == int:
                            self.logger.warning('Pin {} already exists. Cannot override existing pin to make an nwayout pin.'.format(p))
                            return False

...
binding_to_pin_with_pw =  { 
    "s2_to_p1": {
        "p_on": {
            "all": [{"first": {"unique_id": "s2", "payload":  "on"}},
                    {"second": {"unique_id": "pw1", "payload":  "secret"}}],
            "run": {"action":{"unique_id": "p1", "call": "on"} }
        },
        "p_off": {
            "all": [{"third": {"unique_id": "s2", "payload":  "off"}},
                    {"second": {"unique_id": "pw1", "payload":  "secret"}}],
            "run": {"action":{"unique_id": "p1", "call": "off"}  }
        }
    }
...
binding_auto_off =  {
  "delayed_action": {
    "p_on": {
      ...
      "run": [{"timer": { "name": "auto_off", "action": "start", "seconds": 3 }},
  ...
  "s3_to_p1_timed$state": {
    ...
    "waiting": {
      "t_s3_on": {
        ...
        "to": "got_s3_on",
        "run": {"timer": { "name": "PairTimeout", "action": "start", "seconds": 1 }}
      },
      "t_pw_secret": {
    ...
    "got_s3_on": {
      "t_pw_secret": {
        "all": [{"m": {"$and": [{"unique_id": "pw2"}, {"payload": "secret"}]}}],
        "to": "waiting",
        "run": [{"timer": { "name": "PairTimeout", "action": "cancel" }},
                {"action":{ "unique_id": "p1", "call": "on" }} ]
      },
      "t_timeout": {

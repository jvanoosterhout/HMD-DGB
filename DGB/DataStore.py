...
class DataStore:
    ...
    def add_device(
        ...
        self.logger.info(
            "added device {} with functions {}".format(
                unique_id, functions if functions else []
            )
        )

    def add_pin(
        self, unique_id: str, pin_obj: any, functions: dict[str, callable] | None = None
    ):
        self.pins_objects[unique_id] = pin_obj
        self.pins_functions[unique_id] = functions or {}
        self.logger.info(
            "added pin {} with functions {}".format(
                unique_id, functions if functions else []
            )
        )
        ...
        for rule in self.bindings[device_id]:
            if "name" in rule:
                if rule["name"] == ruleset_name:
                    self.logger.info(
                        "Device {} already had binding to ruleset {}".format(
                            device_id, ruleset_name
                        )
                    )
                    return
        self.bindings[device_id].append(ruleset_name)
        self.logger.info(
            "added binding for device {} to ruleset {}".format(device_id, ruleset_name)
        )

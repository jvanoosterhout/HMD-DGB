#
#    Copyright 2024-2025 Jeroen van Oosterhout <18647330+jvanoosterhout@users.noreply.github.com>
#
...
class Pin_api:
    def __init__(
        ...
        async def get_pin_in(pin_config: PinIn = Depends()):
            """
        ...
        async def get_pin_out(pin_config: PinOut = Depends()):
            """
            This endpoint allows to retrieve the curent pin state of the specified existin pin.
            Mind that the configuration of the pin must match the saved configuration. In case
            the pin does not exist (due to e.g. reboot), the pin is created according to the
        ...
        async def get_pin_count(pin_config: PinCount = Depends()):
            """
            This endpoint allows to retrieve the curent pin state of the specified existin pin.
            Mind that the configuration of the pin must match the saved configuration. In case
            the pin does not exist (due to e.g. reboot), the pin is created according to the
        ...
        async def get_pin_nwayout(pin_config: PinNWayOut = Depends()):
            """
            This endpoint allows to retrieve the curent pin state of the specified existin pin.
            Mind that the configuration of the pin must match the saved configuration. In case
            the pin does not exist (due to e.g. reboot), the pin is created according to the
